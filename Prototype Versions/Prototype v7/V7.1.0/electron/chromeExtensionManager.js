...
class ChromeExtensionManager {
  ...
  async downloadAndExtract(extId) {
    const outDir = path.join(this.extensionsDir, extId)
    const url = `https://clients2.google.com/service/update2/crx?response=redirect&prodversion=122.0.0.0&acceptformat=crx2,crx3&x=id%3D${extId}%26uc`
      ...
      const fetchUrl = (targetUrl) => {
        https.get(targetUrl, (res) => {
          ...
          if (res.statusCode !== 200) {
            return reject(new Error(`Failed to download extension CRX (HTTP ${res.statusCode})`))
          }
          ...
          res.on('end', () => {
            ...
            try {
              ...
              resolve({
                ...
                description: manifest.description || 'Chrome Web Store Extension',
                path: outDir,
                manifest,
              })
            } catch (err) {
...
`
  ...
  async installFromStore(input, session) {
    ...
    if (!extId) {
      return { success: false, error: 'Invalid Chrome Web Store URL or Extension ID' }
    }

    try {
      const extData = await this.downloadAndExtract(extId)
      let loaded = null
      if (session) {
        try {
